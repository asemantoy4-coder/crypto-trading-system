...
logging.basicConfig(
    ...
    handlers=[
        logging.StreamHandler(sys.stdout)  # Logs to Render console
    ]
)
...
# ==============================================================================
# Import Utils Module (Optimized)
# ==============================================================================

print("\n[1/3] Importing utils module...")

try:
    # Standard import for root files
...
class ScalperEngine:
    ...
    @staticmethod
    def generate_pro_scalp_targets(
        entry_price: float, 
        signal: str, 
        atr_value: float, 
...
async def get_pro_scalp_signal(config: ScalpConfig):
    ...
    try:
        ...
        if not data or len(data) < 50:
            raise HTTPException(
                status_code=400, 
                detail="Insufficient data for professional scalp analysis"
            )
        ...
        raise HTTPException(
            status_code=500, 
            detail=f"Professional analysis error: {str(e)[:200]}"
        )
