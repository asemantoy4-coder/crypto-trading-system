# api/utils.py - نسخه 7.6.0 با هوشمندسازی قیمت‌گذاری (Smart Entry)
"""
...
+ محاسبه هوشمند قیمت ورود با فیبوناچی و ایچیموکو (Strong Solution)
"""
...
def calculate_smart_entry(data, signal="BUY", strategy="ICHIMOKU_FIBO"):
    ...
    strategy:
        "ICHIMOKU_FIBO": استفاده از سطوح حمایت/مقاومت هر دو سیستم.
        "AGGRESSIVE": قیمت فعلی بازار.
    """
    if not data or len(data) < 30:
        try:
            return float(data[-1][4])  # Fallback to current
        except:
            return 0
    
    # 1. دریافت داده‌های ایچیموکو
    ...
    # 2. دریافت داده‌های فیبوناچی
    swing_high, swing_low = get_swing_high_low(data, lookback=20)
    
    if signal == "BUY":
        # برای خرید، دنبال پایین‌ترین قیمت حمایت هستیم
        # سطوح کاندید:
        ...
        # Fib Support (سطح 0.618 بازگشت): Low + (High-Low)*0.382
        fib_support = swing_low + (swing_high - swing_low) * 0.382
        
        # Fib Deep Support (سطح 0.618 از بالا، یا 0.382 بازگشت کامل): 
        # معمولا برای خرید در کف بهتر است:
        fib_deep_support = swing_low + (swing_high - swing_low) * 0.236
        
        # منطق هوشمند: انتخاب پایین‌ترین سطح حمایتی
        # اما نمی‌خواهیم خیلی پایین بخریم که سفارش زده نشود، پس حداقل را نیز در نظر می‌گیریم
        # بهترین ورود: میانگین وزنی سطوح حمایتی
        candidates_buy = [
            ichimoku_support,
            fib_support,
            fib_deep_support
        ...
        # فیلتر کردن مقادیر نامعتبر و بالاتر از قیمت فعلی (برای خرید نباید بالاتر بخریم مگر بریک‌اوت باشد)
        # اما برای اسکالپ، معمولا لیمیت ارد زیر قیمت است.
        valid_candidates = [c for c in candidates_buy if c < current_price and c > 0]
        
        if valid_candidates:
            # انتخاب پایین‌ترین سطح حمایتی برای خرید ارزان‌تر
            smart_entry = min(valid_candidates)
        else:
            # اگر سطح حمایتی زیر قیمت نبود، کمی زیر قیمت فعلی بخر
            smart_entry = current_price * 0.999 
            
    elif signal == "SELL":
        # برای فروش، دنبال بالاترین سطح مقاومت هستیم
        ichimoku_resistance = max(
            ichimoku_data.get('cloud_top', current_price * 1.01),
            ichimoku_data.get('kijun_sen', current_price * 1.01)
        )
        
        # Fib Resistance (سطح 0.382 بازگشت): High - (High-Low)*0.382
...
def analyze_with_multi_timeframe_strategy(symbol):
    ...
    try:
        ...
        # استفاده از تابع هوشمند برای قیمت ورودی (Strong Solution)
        if signal == "BUY":
            entry_price = calculate_smart_entry(data_5m, signal="BUY")
            stop_loss = entry_price * 0.98  # 2% زیر ورودی
            targets = [entry_price * 1.02, entry_price * 1.05]
        elif signal == "SELL":
            entry_price = calculate_smart_entry(data_5m, signal="SELL")
            stop_loss = entry_price * 1.02  # 2% بالای ورودی
            targets = [entry_price * 0.98, entry_price * 0.95]
        else:
...
__description__ = "سیستم تحلیل معاملاتی با Smart Entry (Fibo + Ichimoku)"
...
print(f"📊 Features: Ichimoku Advanced, Quality Line, Golden Line, Smart Entry (Fibo+Ichimoku)")
