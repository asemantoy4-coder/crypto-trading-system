...
def analyze_ichimoku_scalp_signal(ichimoku_data):
    if not ichimoku_data: return {'signal': 'HOLD', 'confidence': 0.5, 'reason': 'Ichimoku data insufficient', 'levels': {}, 'trend_power': 50}
    ...
    if bullish_conditions >= 3:
        signal = 'BUY'; confidence = min(0.5 + (bullish_conditions * 0.1) + (trend_power / 200), 0.95); reason = f"{reason} (Buy signal)"
    elif bearish_conditions >= 3:
        signal = 'SELL'; confidence = min(0.5 + (bearish_conditions * 0.1) + ((100 - trend_power) / 200), 0.95); reason = f"{reason} (Sell signal)"
    else: signal = 'HOLD'
...
def analyze_with_multi_timeframe_strategy(symbol):
    ...
    try:
        ...
        if bullish_count >= 2: signal, confidence = "BUY", 0.6 + (bullish_count * 0.1)
        elif bearish_count >= 2: signal, confidence = "SELL", 0.6 + (bearish_count * 0.1)
        ...
        else: targets, stop_loss = [], smart_entry
