...
این فایل **`api/utils.py` (نسخه 7.6.0)** کاملاً تمیز، بدون ارور و آماده آپلود است.

**نکات مهم در این نسخه:**
1.  **حذف ارور نحوی:** تمامی توضیحات فارسی خارج از کد حذف شده‌اند تا خطای `SyntaxError: invalid character` رخ ندهد.
2.  **Smart Entry:** منطق قیمت‌گذاری هوشمند (فیبوناچی + ایچیموکو) پیاده‌سازی شده است.
3.  **RSI واقعی:** محاسبه سری کامل RSI برای تشخیص واگرایی.
4.  **واگرایی (Divergence):** تشخیص پیک و دره برای پیدا کردن نقطه بازگشت.

این فایل را کپی کنید و جایگزین محتویات قبلی `api/utils.py` کنید:

```python
# api/utils.py - Version 7.6.0 (Clean & Optimized)
"""
...
def generate_mock_data_simple(symbol, limit=100):
    ...
    for i in range(limit):
        ...
        mock_candle = [
            ...
            str(price * random.uniform(1.000, 1.003)), str(price * random.uniform(0.997, 1.000)),
            str(price), str(random.uniform(1000, 10000)), timestamp + 300000,
            "0", "0", "0", "0", "0"
        ]
...
# ==============================================================================
# 2. Technical Analysis: RSI & Divergence (Real Calculation)
# ==============================================================================
...
def get_swing_high_low(data, period=20):
    """
    دریافت بالاترین قله و پایین‌ترین دره (Swing High/Low) در یک بازه.
    """
...
def calculate_smart_entry(data, signal="BUY", strategy="ICHIMOKU_FIBO"):
    ...
    ترکیب ایچیموکو و فیبوناچی برای پیدا کردن بهترین قیمت ورود.
    """
    if not data or len(data) < 30:
        try: return float(data[-1][4])
        except: return 0
    ...
    if signal == "BUY":
        # Smart Logic for BUY: Lowest Support Level
        ...
        if valid_candidates:
            smart_entry = min(valid_candidates)
        else:
            smart_entry = current_price * 0.999 
            
    elif signal == "SELL":
        # Smart Logic for SELL: Highest Resistance Level
        ...
        if valid_candidates:
            smart_entry = max(valid_candidates)
        else:
            smart_entry = current_price * 1.001
    else:
...
def calculate_ichimoku_components(data, tenkan_period=9, kijun_period=26, senkou_b_period=52, displacement=26):
    ...
    for candle in data:
        try:
            highs.append(float(candle[2])); lows.append(float(candle[3])); closes.append(float(candle[4]))
        except: continue
...
def analyze_ichimoku_scalp_signal(ichimoku_data):
    if not ichimoku_data:
        return {'signal': 'HOLD', 'confidence': 0.5, 'reason': 'ایچیموکو داده ناکافی', 'levels': {}, 'trend_power': 50}
    ...
    if None in [tenkan, kijun, current_price] or current_price <= 0:
        return {'signal': 'HOLD', 'confidence': 0.5, 'reason': 'داده‌های ایچیموکو ناقص', 'levels': {}, 'trend_power': trend_power}
    ...
    signal, confidence, reason = 'HOLD', 0.5, "بازار خنثی"
    bullish_conditions, bearish_conditions = 0, 0
    
    if tenkan_above_kijun:
        bullish_conditions += 1; reason = "تنکان بالای کیجون"
    if price_above_tenkan and price_above_kijun:
        bullish_conditions += 1; reason = "قیمت بالای هر دو خط"
    if price_above_cloud:
        bullish_conditions += 2; reason = "قیمت بالای ابر کومو"
    if not tenkan_above_kijun:
        bearish_conditions += 1; reason = "تنکان زیر کیجون"
    if not price_above_tenkan and not price_above_kijun:
        bearish_conditions += 1; reason = "قیمت زیر هر دو خط"
    if cloud_bottom and current_price < cloud_bottom:
        bearish_conditions += 2; reason = "قیمت زیر ابر کومو"
    ...
    levels = {
        'tenkan_sen': round(tenkan, 4), 'kijun_sen': round(kijun, 4),
        'cloud_top': round(cloud_top, 4) if cloud_top else None,
        'cloud_bottom': round(cloud_bottom, 4) if cloud_bottom else None,
        'quality_line': round(ichimoku_data.get('quality_line'), 4),
        'golden_line': round(ichimoku_data.get('golden_line'), 4),
    ...
    trend_interpretation = "روند قوی" if trend_power >= 70 else "روند متوسط" if trend_power >= 60 else "روند ضعیف" if trend_power >= 40 else "بدون روند"
    
    return {
        'signal': signal, 'confidence': round(confidence, 3), 'reason': reason, 'levels': levels,
        'trend_power': trend_power, 'trend_interpretation': trend_interpretation,
        'cloud_thickness': ichimoku_data.get('cloud_thickness', 0), 'in_cloud': price_in_cloud,
...
def analyze_with_multi_timeframe_strategy(symbol):
    ...
    try:
        ...
        # Use Smart Entry for consistent pricing
        smart_entry = calculate_smart_entry(data_5m, signal)
        
        if signal == "BUY":
            targets = [smart_entry * 1.02, smart_entry * 1.05]
            stop_loss = smart_entry * 0.98
        elif signal == "SELL":
            targets = [smart_entry * 0.98, smart_entry * 0.95]
            stop_loss = smart_entry * 1.02
        else:
            targets = []
            stop_loss = smart_entry
        
        return {
            "symbol": symbol, "signal": signal, "confidence": round(min(confidence, 0.95), 2),
            "entry_price": round(smart_entry, 2), "targets": [round(t, 2) for t in targets],
            "stop_loss": round(stop_loss, 2), "strategy": "Multi-Timeframe Smart Entry",
            "analysis_details": {
...
def get_fallback_signal(symbol):
    ...
    if signal == "BUY":
        targets, stop_loss = [round(entry_price * 1.02, 2), round(entry_price * 1.05, 2)], round(entry_price * 0.98, 2)
    elif signal == "SELL":
        targets, stop_loss = [round(entry_price * 0.98, 2), round(entry_price * 0.95, 2)], round(entry_price * 1.02, 2)
    else:
        targets, stop_loss = [], entry_price
    
    return {
        "symbol": symbol, "signal": signal, "confidence": confidence, "entry_price": entry_price,
        "targets": targets, "stop_loss": stop_loss, "strategy": "Fallback Mode",
...
def analyze_scalp_conditions(data, timeframe):
    if not data or len(data) < 20:
        return {"condition": "NEUTRAL", "rsi": 50, "sma_20": 0, "volatility": 0, "reason": "Insufficient data"}
    
    rsi, sma_20 = calculate_simple_rsi(data, 14), calculate_simple_sma(data, 20)
    ...
    if latest_close <= 0 or sma_20 <= 0:
        return {"condition": "NEUTRAL", "rsi": round(rsi, 1), "sma_20": 0, "current_price": 0, "volatility": 0, "reason": "Invalid price data"}
    
    if rsi < 30 and latest_close < sma_20 * 1.01:
        condition, reason = "BULLISH", f"Oversold (RSI: {rsi:.1f}), price near SMA20"
    elif rsi > 70 and latest_close > sma_20 * 0.99:
        condition, reason = "BEARISH", f"Overbought (RSI: {rsi:.1f}), price near SMA20"
    elif latest_close > sma_20 * 1.02 and rsi < 60:
        condition, reason = "BULLISH", f"Breakout above SMA20, RSI: {rsi:.1f}"
    elif latest_close < sma_20 * 0.98 and rsi > 40:
        condition, reason = "BEARISH", f"Breakdown below SMA20, RSI: {rsi:.1f}"
    elif volatility > 1.0 and timeframe in ["1m", "5m"]:
        condition, reason = "VOLATILE", f"High volatility: {volatility:.2f}%"
    
    return {
        "condition": condition, "rsi": round(rsi, 1), "sma_20": round(sma_20, 2),
        "current_price": round(latest_close, 2), "volatility": round(volatility, 2), "reason": reason
    }
...
def generate_ichimoku_recommendation(signal_data):
    signal, confidence = signal_data.get('signal', 'HOLD'), signal_data.get('confidence', 0.5)
    in_cloud, trend_power = signal_data.get('in_cloud', False), signal_data.get('trend_power', 50)
    recommendations = {
        'BUY': {'high': 'Strong Buy', 'medium': 'Medium Buy', 'low': 'Weak Buy'},
        'SELL': {'high': 'Strong Sell', 'medium': 'Medium Sell', 'low': 'Weak Sell'},
        'HOLD': {'in_cloud': 'Wait in cloud', 'low_conf': 'Stay away', 'default': 'Hold'}
    }
    if signal == 'BUY':
...
def combined_analysis(data, timeframe="5m"):
    ...
    results = {
        'rsi': calculate_simple_rsi(data, 14), 'sma_20': calculate_simple_sma(data, 20),
        'macd': calculate_macd_simple(data), 'ichimoku': get_ichimoku_scalp_signal(data, timeframe),
        'support_resistance': get_support_resistance_levels(data), 'volatility': calculate_volatility(data, 20)
    }
    ...
    return {
        'signal': final_signal.upper(), 'confidence': round(confidence, 3), 'details': results,
        'price': latest_price, 'timestamp': datetime.now().isoformat()
    }
...
__all__ = [
    'get_market_data_with_fallback', 'analyze_with_multi_timeframe_strategy',
    'calculate_simple_sma', 'calculate_simple_rsi', 'calculate_rsi_series', 'detect_divergence',
    'calculate_macd_simple', 'analyze_trend_simple', 'get_swing_high_low', 'calculate_smart_entry',
    'calculate_ichimoku_components', 'analyze_ichimoku_scalp_signal', 'get_ichimoku_scalp_signal',
    'calculate_quality_line', 'calculate_golden_line', 'calculate_24h_change_from_dataframe',
    'analyze_scalp_conditions', 'get_support_resistance_levels', 'calculate_volatility',
    'combined_analysis', 'generate_ichimoku_recommendation', 'get_fallback_signal'
]
