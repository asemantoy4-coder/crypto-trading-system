"""
Crypto AI Trading System v7.6.5 (FIXED TARGETS CALCULATION)
Fixed:
1. Lambda lookback argument error
2. All function signatures matched
3. Correct targets and stop loss calculation for BUY/SELL signals
4. Added proper percentage calculations
...
def mock_analyze_with_multi_timeframe_strategy(symbol):
    ...
    # Fixed targets and stop loss calculation
    if signal == "BUY": 
        targets = [
...
async def get_scalp_signal(request: ScalpRequest):
    ...
    try:
        ...
        logger.info(f"🔍 Signal: {scalp_analysis['signal']}, Entry: {smart_entry_price}, "
                   f"RSI: {scalp_analysis['rsi']}, SMA20: {scalp_analysis.get('sma_20', 0)}")
        ...
        if scalp_analysis["signal"] == "BUY":
            ...
            targets = [
                round(smart_entry_price * 1.01, 8),   # TP1: +1%
                round(smart_entry_price * 1.015, 8),  # TP2: +1.5%
                round(smart_entry_price * 1.02, 8)    # TP3: +2%
            ]
            stop_loss = round(smart_entry_price * 0.99, 8)   # SL: -1%
            logger.info(f"📈 BUY Signal - Targets: {targets}, SL: {stop_loss}")
            
        elif scalp_analysis["signal"] == "SELL":
            # For SELL: TP below entry, SL above entry
            targets = [
                round(smart_entry_price * 0.99, 8),   # TP1: -1%
                round(smart_entry_price * 0.985, 8),  # TP2: -1.5%
                round(smart_entry_price * 0.98, 8)    # TP3: -2%
            ]
            stop_loss = round(smart_entry_price * 1.01, 8)   # SL: +1%
            logger.info(f"📉 SELL Signal - Targets: {targets}, SL: {stop_loss}")
            
        else: 
            # For HOLD signal - Conservative targets
            targets = [
                round(smart_entry_price * 1.005, 8),  # +0.5%
                round(smart_entry_price * 1.01, 8),   # +1%
                round(smart_entry_price * 1.015, 8)   # +1.5%
            ]
            stop_loss = round(smart_entry_price * 0.995, 8)  # -0.5%
            logger.info(f"⏸️ HOLD Signal - Targets: {targets}, SL: {stop_loss}")
        ...
        logger.info(f"✅ Generated scalp signal: {response['signal']} for {response['symbol']} "
                   f"(Confidence: {response['confidence']}, RSI: {response['rsi']}, Risk: {response['risk_level']})")
        ...
        if mock_signal == "BUY":
            targets = [
                round(entry_price * 1.01, 8),   # +1%
                round(entry_price * 1.015, 8),  # +1.5%
                round(entry_price * 1.02, 8)    # +2%
            ]
            stop_loss = round(entry_price * 0.99, 8)   # -1%
        elif mock_signal == "SELL":
            targets = [
                round(entry_price * 0.99, 8),   # -1%
                round(entry_price * 0.985, 8),  # -1.5%
                round(entry_price * 0.98, 8)    # -2%
            ]
            stop_loss = round(entry_price * 1.01, 8)   # +1%
        else:
            targets = [
