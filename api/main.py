"""
Crypto AI Trading System v7.6.4 (FIXED TARGETS VERSION)
Fixed:
1. Lambda lookback argument error
2. All function signatures matched
3. Correct targets and stop loss calculation
4. Added risk level and percentages
    ...
    try:
        ...
        if scalp_analysis["signal"] == "BUY":
            ...
            targets = [
                ...
                round(smart_entry_price * 1.02, 8),  # TP2: +2%
                round(smart_entry_price * 1.03, 8)   # TP3: +3%
            ]
            ...
            targets = [
                ...
                round(smart_entry_price * 0.98, 8),  # TP2: -2%
                round(smart_entry_price * 0.97, 8)   # TP3: -3%
            ]
        ...
        tp1_percent = round(((targets[0] - smart_entry_price) / smart_entry_price) * 100, 2) if targets else 0
        tp2_percent = round(((targets[1] - smart_entry_price) / smart_entry_price) * 100, 2) if len(targets) > 1 else 0
        tp3_percent = round(((targets[2] - smart_entry_price) / smart_entry_price) * 100, 2) if len(targets) > 2 else 0
        ...
        logger.info(f"✅ Generated scalp signal: {response['signal']} for {response['symbol']} "
                   f"(Confidence: {response['confidence']}, RSI: {response['rsi']})")
