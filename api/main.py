"""
Crypto AI Trading System v7.4 - Render Optimized & Final Version
با پشتیبانی از Binance/LBank API و سازگار با پلتفرم Render
نسخه کامل با پشتیبانی از ایچیموکو پیشرفته برای اسکالپ
"""
...
if UTILS_AVAILABLE:
    ...
    print("✅ Ichimoku functions available")
else:
...
# FastAPI Application
# ==============================================================================
API_VERSION = "7.4.0"  # نسخه جدید با پشتیبانی از ایچیموکو

app = FastAPI(
    title=f"Crypto AI Trading System v{API_VERSION}",
    description=f"Multi-source signal API with Ichimoku Support - نسخه {API_VERSION}",
    version=API_VERSION,
    docs_url="/api/docs",
    redoc_url="/api/redoc"
)
...
# ==============================================================================
# توابع کمکی جدید برای اسکالپ و ایچیموکو
# ==============================================================================
...
def analyze_ichimoku_scalp(symbol, timeframe, data):
    ...
    try:
        ...
        if ichimoku_signal['signal'] == 'BUY':
            # حمایت: پایین‌ترین سطح از بین خطوط
            support = min(
            ...
            # مقاومت: بالاترین سطح از بین خطوط
            resistance = max(
                ichimoku_data.get('cloud_top', current_price * 1.01),
...
async def read_root():
    ...
    endpoints = {
        ...
        "ichimoku_scalp": "POST /api/ichimoku-scalp",  # جدید
        "combined_analysis": "POST /api/combined-analysis",  # جدید
    ...
    return {
        ...
        "note": f"نسخه {API_VERSION} با پشتیبانی از ایچیموکو پیشرفته"
    }
...
async def get_ichimoku_scalp_signal(request: IchimokuRequest):
    ...
    شامل خط کیفیت و خط طلایی
    """
    logger.info(f"☁️ درخواست سیگنال ایچیموکو: {request.symbol} ({request.timeframe})")
    
    # تایم‌فریم‌های مجاز (می‌تواند بیشتر از اسکالپ معمولی باشد)
